...
$sql.=" FROM lg_cabeza T1  INNER JOIN  empleadolg T2 ON  T1.identificacion = T2.cedula where estado = 'APR'";   
...
if( !empty($requestData['search']['value']) ) {
	...
	$sql.=" WHERE T1.id_cabeza LIKE '".$requestData['search']['value']."%'  ";    // $requestData['search']['value'] contains search parameter
	$sql.=" OR T1.tipolegalizacion LIKE '%".$requestData['search']['value']."%' ";
	$sql.=" OR T1.fecha LIKE '%".$requestData['search']['value']."%' ";
    $sql.=" OR T1.linea LIKE '%".$requestData['search']['value']."%' ";
    $sql.=" OR T1.area LIKE '%".$requestData['search']['value']."%' ";
	$sql.=" OR T1.estado LIKE '%".$requestData['search']['value']."%' ";
	$sql.=" OR T1.nombre LIKE '%".$requestData['search']['value']."%' ";
	$sql.=" OR T2.regional LIKE '%".$requestData['search']['value']."%' ";
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	if($row["estado"] != "50"){
		$nestedData[] = '<td ><center>                 
						 <a href="/scandinavia/aplicaciones/legalizaciones/verdocumentosC.php?id='.$row['id_cabeza'].'&op=LG"   title="Soportes" class="btn"  style="background-color:#00AB84;"> <i class="menu-icon glyphicon glyphicon-search" style="color:white;"></i> </a>                     
						</center></td>';
		}else{
		$nestedData[] = '<td><center>	                 
		...
		$nestedData[] = '<td ><center>
						 <a href="/scandinavia/aplicaciones/legalizaciones/verdocumentosC.php?id='.$row['id_cabeza'].'&op=LG"   title="Ver Documento" class="btn"  style="background-color:#00AB84;"> </a>                     
						 </center></td>';    				 			
		$data[] = $nestedData;    
	}
