...
while($row = mysqli_fetch_array($result))
{
   ...
   $nombre = $row['full_name'];
    ...
    $sql="UPDATE lg_cabeza SET estado = '$estadoscartera', observacionP = '$observaciones', usermodifica = $usuariomodifica , fechacontabilidad = NOW() Where id_cabeza = $recibo";
...
if($estadoscartera == "APRC")
{
            ...
            $asunto = "Legalización ".$recibo." ha sido aprobada por contabilidad";
...
else if($estadoscartera == "RECC")
{
            ...
            $mail->IsHTML(true); // El correo se envía como HTML
            $asunto = "Legalización ".$recibo." ha sido rechazada por contabilidad";
            $mail->Subject = utf8_decode($asunto); // Este es el titulo del email.
            $body = " Estimado/a  $nombre, <br><br>
            El estado de la legalización $recibo es <strong>$estadoscartera</strong>
...
if($estadoscartera == 'RECC'){
$sql="UPDATE lg_cabeza SET  boton = '1', observacionP = '$observaciones', fechacontabilidad = NOW() Where id_cabeza = $recibo";
mysqli_query($mysqli, $sql);
}



header('Location: /scandinavia/aplicaciones/legalizaciones/listadolegalizacionesc.php');	
?>