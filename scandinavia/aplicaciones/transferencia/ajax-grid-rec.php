...
// getting total number records without any search
	$sql = "SELECT id_transferencia, fecha_registro, establecimiento, descripcion, estado FROM transferencia_val where estado = 2";
...
if($row["estado"] == "1") { $estadoDoc = "En proceso"; }else if($row["estado"] == "2"){$estadoDoc = "Sin distribuir";} else { $estadoDoc = $row["estado"]; $verlink = "pdfok";}
	...
	if($row["estado"] != "NO ACEPTO" && $row['estado'] != "REC" && $row['estado'] != "2" ){
		$nestedData[] = '<td><center>	                 
			...
			$nestedData[] = '<td><center>	                 
			<a href="redistribuir.php?documento='.$row['id_transferencia'].'"   title="Ver Documento" class="btn" style="background-color:#00AB84;"><i class="menu-icon glyphicon glyphicon-search" style="color:white;"></i>  </a>                     
			</center></td>'; 
