...
if( !empty($requestData['search']['value']) ) {
	...
	$sql.=" GROUP BY consecutivo ORDER BY estado, ". $columns[$requestData['order'][0]['column']]."   ".$requestData['order'][0]['dir']."   LIMIT ".$requestData['start']." ,".$requestData['length']."   ";
...
if($row["estado"] == "1") { $estadoDoc = "Sin Verificar"; } else { $estadoDoc = $row["estado"]; $verlink = "pdfok";} 
	...
	$nestedData[] = '<td><center>	                 
                     <a href="/scandinavia/aplicaciones/Anticipos/verdocumentosC.php?id='.$row['consecutivo'].'&op=LG"   title="Soportes" class="btn" style="background-color:#00AB84;"> <i class="menu-icon glyphicon glyphicon-search" style="color:white;"></i> </a>                     
				     </center></td>';
	}else{
	$nestedData[] = '<td><center>	                 
    ...
    $nestedData[] = '<td><center>
                     <a href="/scandinavia/aplicaciones/Anticipos/verdocumentosC.php?id='.$row['consecutivo'].'&op=LG"  title="Ver Documento" class="btn" style="background-color:#00AB84;"> <i class="menu-icon glyphicon glyphicon-search"></i> </a>                     
				     </center></td>';    				 			
	$data[] = $nestedData;    
}
