...
$sql = "SELECT T1.consecutivo, T1.tipo, T1.fechaActual, T1.identificacion, T1.nombre, T1.estado FROM anticipo T1 INNER JOIN system_users T2 WHERE T1.estado = '0' and identificacion = ".$identificacion." group by T1.consecutivo";
...
if($row["estado"] == "0") { $estadoDoc = "Sin finalizar"; } else { $estadoDoc = $row["estado"]; $verlink = "pdfok";}
	...
	$nestedData[] =        $estadoDoc;
		...
		$nestedData[] = '<td><center>	                 
						<a href="/scandinavia/aplicaciones/Anticipos/indexEmpleado2.php?documento='.$row['consecutivo'].'&op=LISTADO ANTICIPOS"   title="Ver Documento" class="btn" style="background-color:#00AB84;"> <i class="menu-icon glyphicon glyphicon-search" style="color:white;"></i></a>                     
						</center></td>';
		}else{
		$nestedData[] = '<td><center>	                 
		...
		$nestedData[] = '<td><center>
						<a href="/scandinavia/aplicaciones/Anticipos/indexEmpleado2.php?documento='.$row['consecutivo'].'&op=LG"   title="Ver Documento" class="btn btn-sm btn-primary"> <i class="menu-icon glyphicon glyphicon-search"></i></a>             
						</center></td>';    				 			
		$data[] = $nestedData;    
