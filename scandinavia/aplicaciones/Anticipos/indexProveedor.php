...
$condition['where'] = array('cedulaSesion'=> $cedulaSesion, 'modulo' => 'Anticipos',); 
$usuario = $db->getRows('matrizaprobacion',$condition);

$condicion['where'] = array('u_userid'=> $usuario[0]['id_Aprobador'],); 
$apr = $db->getRows('system_users',$condicion); //ojo se pone tabla a consultar


if($usuario[0]['modulo'] === "Anticipos")
{
    $idAprobador = $usuario[0]['id_Aprobador'];

}
else
{
    echo "No tiene asignado ningún permiso aun";
}
            ...
            $(document).ready(function() {
                ...
                $("#my-form").submit(function(e) {
                    ...
                    $.ajax( {
                        ...
                        url: "guardarajaxproveedores.php",
                        method: "post",
                        //data: $("form").serialize(),
...
/*$(document).ready(function(){
    ...
    var value = $(this).val();
    if($(this).maskMoney('unmasked')[0] === 7000000){
        if($("#file").length === 0){
            $("#subida").append("<label for='ex2' id='ar'>Adjuntar póliza</label>");
            $("#subida1").append("<input id='file' name='file[]' type='file' required='required'>");
        }    	
    }else{
        $("#file").remove(); 
        $("#ar").remove(); 
...
function eliminarsolicitud2(idanticipo, consecutivo ){
	...
	if ( eliminar ) {
		...
		ajax.open("GET", "eliminaproveedor.php?documento="+idanticipo,true); 
...
function invima() 
{
    let nitInvima = $('#nit1').val();    
    if(nitInvima == "830000167-2" || nitInvima == "901154297-1")
    {
        $('#confirmacion').append('<input type="hidden" name="confirmacionCompras" id="comp" value="true">');
        $('#confirmacion').append('<input type="hidden" name="confirmacionGerencia" id="geren" value="true">');
        let prueba1 = $('#comp').val();
        let prueba2 = $('#geren').val();
    ...
    else
    {
        $('#comp').remove();
        $('#geren').remove();
        let prueba1 = $('#comp').val('false');
        let prueba2 = $('#geren').val('false');
...
$(document).ready(function(){
    $("#inversioncom1").change(function(){
        ...
        if($("#texto").length === 0){
            ...
            $("#info2").append("<input class='form-control' name='cinversion' style='text-transform:uppercase' id='texto1' type='text'placeholder='C.inversión'>");
   ...
   <div class="panel" style="border:1px solid #00AB84;">
                            ...
                            <div class="row">
                                                            ...
                                                            <div class="col-md-3 ">
                                                                            <div class="form-group">
                                                                                ...
                                                                                <div class="row">
                                                			    			        <div class="col-md-8">
                                                                                    <input type="text" class="form-control form-control-lg" name="fechadesembolso" id="dateArrival1" placeholder="Seleccionar Fecha" >
	...
	<div class="panel" style="border:1px solid #00AB84;" >            
			...
			<div class="row">
				...
				<div class="col-md-6" id="apro1"><h5>El aprobador de esta legalizacion es: <strong><?=$apr[0]['full_name']?></strong></h5> </div>				
            ...
            <input name="cedulaSesion" type="hidden" id="cedulaSesion" value="<?= $idAprobador?>" /> 
                <input name="username" type="hidden" id="namel" value="<?=$_SESSION['session_name'];?>" />
                    </div>
                <div class="col-md-10">

                    <input name="input" type="submit" id="buttonsave"  class=" btn" style="color:white; background-color:#00AB84;" value="Enviar Información" /> 
