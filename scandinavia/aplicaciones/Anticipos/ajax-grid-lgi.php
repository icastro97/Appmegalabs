...
?>
<form method="post" action="finalizadoc.php" enctype="multipart/form-data"> 
...
while( $row_Transacciones=mysqli_fetch_array($query) ) {  // preparing an array
 $identificacion = $row_Transacciones['identificacion'];
	echo "	<tr>";
...
while( $row_Transacciones2=mysqli_fetch_array($query2) ) {  // preparing an array
	...
	if($total >= 5000000 && $identificacion != "830000167-2")
	{
		...
		echo "<br>";
		echo "<br>";		
		echo "<button type='submit' class='btn' style='background-color:#00AB84; color:white;' disabled='disabled'>Finalizar documento</button>";
		
	}
	else
	{
		echo "<button type='submit' class='btn' style='background-color:#00AB84; color:white;'>Finalizar documento</button>";
	}
