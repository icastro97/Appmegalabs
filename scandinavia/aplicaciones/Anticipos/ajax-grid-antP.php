...
$sql.=" FROM anticipo where estado <> '0' and tipo = 'Proveedor' GROUP BY consecutivo";
...
if($row["estado"] == "1") { $estadoDoc = "Sin Verificar"; } else { $estadoDoc = $row["estado"]; $verlink = "pdfok";} 
	...
	$nestedData[] = '<td><center>	                 
                     <a href="/scandinavia/aplicaciones/Anticipos/verdocumentosP.php?id='.$row['id_anticipo'].'&op=LG"   title="Soportes" class="btn btn-sm btn-primary"> <i class="menu-icon glyphicon glyphicon-search"></i> </a>                     
				     </center></td>';
	}else{
	$nestedData[] = '<td><center>	                 
    ...
    $nestedData[] = '<td><center>
                     <a href="/scandinavia/aplicaciones/Anticipos/verdocumentosP.php?id='.$row['id_anticipo'].'&op=LG"   title="Ver Documento" class="btn btn-sm btn-primary"> <i class="menu-icon glyphicon glyphicon-search"></i> </a>                     
				     </center></td>';    				 			
	$data[] = $nestedData;    
}
