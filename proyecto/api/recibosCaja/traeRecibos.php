...
$id = $_GET['id'];
$ped = [];
$sql = "SELECT T1.DocumentID, T1.Documento, T1.ValorDocumento, T1.RetImpositiva, T1.DesPPago, T1.documentoOrigi, T1.DesAdicional, T1.OtrosDescuentos, T1.NoNota, T1.ValNota, T1.ValNeto, T1.DiasPago, T1.TiempoPago, T1.Habilitado, T2.Cliente, T2.cod, T2.ValAntesIva, T2.Saldo, T2.VtoFecha FROM `rd_detalle` 
T1 INNER JOIN vw_basec_vendedores T2 ON T1.Documento = T2.Documento WHERE T1.DocumentID ='$id'";
//var_dump($sql);
...
if($result)
{
  ...
  while($row = mysqli_fetch_assoc($result))
  {
    ...
    $ped[$i]['DocOrig']    = $row['documentoOrigi'];
